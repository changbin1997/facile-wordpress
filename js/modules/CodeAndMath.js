...
export default class CodeAndMath {
  ...
  highlight() {
    ...
    if ($('body').hasClass('enable-highlight') && $('pre').length) {
      // highlight.js 的加载地址，已输出到 body 的 data-hljs-url
      const hljsUrl = `${window.themeConfig.themeUri}/js/highlight.pack.js`;
      ...
      const highlightInit = () => {
        for (let i = 0;i < $('pre').length;i ++) {
          ...
          if ($('pre').eq(i).children('code').length) {
            ...
            btnEl.setAttribute('data-original-title', facileTranslations.copyCodeBtn);
